...
): BaseFlowUseCase<ValidateDataUseCase.Param, Resource<ValidateUtilityEntity>>() {
    ...
    override fun run(param: Param?): Flow<Resource<ValidateUtilityEntity>> = flow {
        ...
        try {
            param?.let {
                ...
                val response  = clientRepository.validateDataCustomer(it.type, map, "Bearer " + user.token)
