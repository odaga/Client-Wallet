...
class ValidateTVUseCase @Inject constructor(
    val clientRepository: ClientRepository,
    val preferenceRepository: PreferenceRepository,
    val utilRepository: UtilRepository
) : BaseFlowUseCase<ValidateTVUseCase.Param, Resource<ValidateUtilityEntity>>() {
    ...
    override fun run(param: Param?): Flow<Resource<ValidateUtilityEntity>> = flow {
        ...
        try {
            param?.let {
                ...
                val response =
                    clientRepository.validateTVCustomer(it.type, map, "Bearer " + user.token)
