...
class DepositToWalletUseCase @Inject constructor(
    ...
    BaseFlowUseCase<DepositToWalletUseCase.Param, Resource<String>>() {

    data class Param(
        val device_id:String,
        val model:String,
        val mobile_account:String,
        val amount:String,
    ...
    override fun run(param: Param?): Flow<Resource<String>> = flow {
        ...
        try {
            ...
            param?.let {
                ...
                val response = client.confirmDepositToWallet(hashMap, "Bearer " + user.token)
